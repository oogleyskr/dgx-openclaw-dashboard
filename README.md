...
health, 4 multimodal AI services, Discord bot status, and SSH tunnel — all
...
- **Model Server Health** -- SGLang provider health check with latency
  and consecutive failure tracking
- **Multimodal Services** -- Health status of 4 local AI services (STT, Vision,
  TTS, ImageGen) with model names and port numbers
...
| RTX 3090          | 24 GB GDDR6X, runs 4 multimodal AI services     |
| OS                | Ubuntu 24.04 (DGX), WSL2 Ubuntu (host)          |
| LLM               | gpt-oss-120b MXFP4 via SGLang                    |
| Multimodal        | STT, Vision, TTS, ImageGen                       |
...
- Multimodal services running on ports 8101-8104 (optional — shows DOWN if not running)
...
| `GET /api/provider`  | Gateway RPC           | SGLang health, latency, failures     |
...
| `GET /api/multimodal`| Local HTTP            | 4 multimodal services health         |
...
dgx-openclaw-dashboard/
  ...
  public/
    index.html                Dashboard web UI (dark theme, 7 cards + log)
  docs/
